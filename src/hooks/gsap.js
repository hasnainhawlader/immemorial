...
export const useGsapUnvell = (item, duration = 2, trigger) => {
  useEffect(() => {
...
export const useGsapDownStagger = (arr, duration = 1.5) => {
  useEffect(() => {
    arr.map((item) => {
      gsap.fromTo(
        item.current,
...
export const useGsapPhotoDropping = (pArr) => {
  useEffect(() => {
    const el = pArr.map((photo) => photo.current);
